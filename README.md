...
- `scripts/refs.json` — all 172 papers with DOIs and their framework-layer routing
- `scripts/still_missing.json` — the ~79 that are paywalled, with DOIs
...
`scripts/remap_modules.py` is the corpus routing table — one auditable line per paper, mapping
each to its framework layer.
...
- **Scripts** (`scripts/`, `course/restyle.py`) — MIT
